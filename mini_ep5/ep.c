...
void *find(void *t_data) {
    struct thread_data *data = (struct thread_data *) t_data;

	obj st = createState(data->search);

	for (int i = 0; i < data->len - 1; i++) {
		if (advance(st, data->array[i], data->array[i+1])) {
			data->count++;
		}
	}
    /* printf("FIND******************888\n"); */
    /* printf("id = %d\ncount = %d\nlen %ld\nsearch %s\n", data->thread_id, data->count, data->len, data->search); */
	freeState(st);
    return NULL;
    /* pthread_exit(NULL); */
}
...
int task(char *data, long len, char *search) {
    ...
    // alocando tudo
    thread_array = malloc(sizeof(pthread_t) * threads);
    thread_data_array = malloc(sizeof(struct thread_data) * threads);

    data_block = len / threads;

	for (int t = 0; t < threads; t++) {
        // inicializando
        thread_data_array[t].thread_id = t;
        thread_data_array[t].count = 0;
        thread_data_array[t].array = data + (data_block*t);
        thread_data_array[t].len = data_block;
        thread_data_array[t].search = search;

        /* printf("t = %d ***********************\n", t); */
        /* printf("id = %d\ncount = %d\nlen %ld\nsearch %s\n", thread_data_array[t].thread_id, thread_data_array[t].count, thread_data_array[t].len, thread_data_array[t].search); */


        pthread_create(&thread_array[t], NULL, find, (void*) &thread_data_array[t]);
	}
    ...
    for (int i = 0; i < threads; i++) {
        ...
        count += thread_data_array[i].count;
    }
